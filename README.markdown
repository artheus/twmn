...
`twmnd`: daemon listening to notification requests and showing them one after another. Configure it at `~/.twmn`. The file is generated the first time `twmnd` is launched.
